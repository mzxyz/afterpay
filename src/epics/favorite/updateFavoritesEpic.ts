...
const updateFavoriteList = (storeItem: TStoreItem, favoriteList: TStoreItem[]) => {
	...
	const storeList = isFavorite
		? [...favoriteList, storeItem]
...
const updateItemFavoriteStatus = (storeItem: TStoreItem, storeList: TStoreItem[]) => {
	...
	return storeList.map((item) => ({
		...item,
		isFavorite: item.id === id ? isFavorite : item.isFavorite,
	}));
};

const getUpdatedStoreList = (
	category: string,
	storeItem: TStoreItem,
	storeList: TStoreItem[],
): TStoreItem[] =>
	category === 'Favorites'
		? updateFavoriteList(storeItem, storeList)
		: updateItemFavoriteStatus(storeItem, storeList);
